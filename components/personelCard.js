			...
			<View style={{ flex: 1 }}>
				<View style={styles.content}>
					{this.props.data.map((data, i, arr) => {
						...
						if (data.resim == 'null') {
							img = 'https://github.com/partitect/cobi/blob/master/null-img1.png';
						} else {
							img = data.resim;
						}
						const App = () => {
							return (
								<TouchableRipple
									onPress={() => this.onPress(data.ad, data.id, this.props.navScreen, this.props.Tur)}
									style={{
								...
								>
									...
									>
										...
										>
											<Headline style={[ styles.imgText ]}>{data.ad}</Headline>
										</LinearGradient>
									</ImageBackground>
								</TouchableRipple>
							);
