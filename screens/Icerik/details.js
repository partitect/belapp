...
class Details extends React.Component {
	...
	componentWillMount() {
		fetch('https://raw.githubusercontent.com/partitect/cobi/master/icerikler.json')
			.then((response) => response.json())
			.then((responseJson) => {
				this.setState({ datas: responseJson.icerikler[0].haberler[4], isLoading: true });
			})
			.catch((error) => {});
	...
	renderContent = () => (
		...
		>
			<Text style={styles.infoTitle}>{this.state.datas.title}</Text>
			<Text style={styles.priceText}>{this.state.datas.subtitle}</Text>
		</View>
	);
	render() {
		return (
