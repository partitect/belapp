...
class HizmetDetails extends React.Component {
	...
	componentWillMount() {
		fetch('https://raw.githubusercontent.com/partitect/cobi/master/hizmetler.json')
			.then((response) => response.json())
			.then((responseJson) => {
				this.setState({ datas: responseJson.hizmetler[0], isLoading: true });
			})
			.catch((error) => {});
	...
	renderContent = () => (
		...
		>
			<Text style={styles.infoTitle}>{this.state.datas.ad}</Text>
			<Text style={styles.infoTitle}>Buraya Hizmetlerle ilgili detaylar gelecek. Bugün doğanlar, vefat edenler v.s.</Text>
		...
		return (
			<View flex={1}>
				...
				{this.state.isLoading ? (
					<StickyHead
						Title={this.state.datas.ad}
						Image="https://image.freepik.com/free-photo/flag-turkey_1401-245.jpg"
...
export default withTheme(HizmetDetails);
